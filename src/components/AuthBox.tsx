...
export default function AuthBox() {
  ...
  const handleForgotPassword = async () => {
    if (!email) {
      setErrorMsg("Por favor, ingresa tu correo electrónico para recuperar tu contraseña.");
      return;
    }
    ...
    try {
      ...
      // Mensaje de éxito informando que se envió el código
      setErrorMsg('');
      setIsResettingPassword(true);
    } catch (err: any) {
      console.log("Clerk Forgot Password Error:", err.errors || err);
      setErrorMsg(getErrorMessage(err, 'generic_login'));
    } finally {
      setIsLoading(false);
    }
  ...
  const handleResetPasswordConfirm = async (e: React.FormEvent) => {
    ...
    if (!resetCode || !newPassword) {
      setErrorMsg("Por favor, ingresa el código y la nueva contraseña.");
      return;
    }
    ...
    try {
      ...
      if (result.status === 'complete') {
        ...
        console.log("Password reset needs additional steps:", result);
        setErrorMsg(t.errors.additional_steps);
        setIsLoading(false);
      }
    } catch (err: any) {
      console.log("Clerk Reset Password Confirm Error:", err.errors || err);
      setErrorMsg(getErrorMessage(err, 'generic_login'));
      setIsLoading(false);
    }
  };
  ...
  if (isResettingPassword) {
    return (
      <div className="bg-white/85 backdrop-blur-md p-8 rounded-3xl shadow-2xl max-w-md w-full border border-white/20">
        ...
        <h2 className="text-2xl font-bold text-center text-gray-800 mb-2">Cambiar Contraseña</h2>
        <p className="text-sm text-gray-600 text-center mb-6">
          Ingresa el código de 6 dígitos que enviamos a <strong>{email}</strong> y elige tu nueva contraseña.
        </p>
        ...
        <div className="mt-6 text-center">
          <button 
            type="button" 
            onClick={() => { setIsResettingPassword(false); setErrorMsg(''); }}
            className="text-sm text-gray-500 hover:text-emerald-600 font-medium underline transition-colors focus:outline-none"
          >
        ...
        <div className="relative">
          ...
          {isLogin && (
            <div className="flex justify-end mt-1">
              <button type="button" onClick={handleForgotPassword} className="text-xs text-emerald-600 font-semibold hover:underline">
                {t.forgotPassword}
              </button>
            </div>
          )}
